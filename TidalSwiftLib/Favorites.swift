...
public class Favorites {
	...
	public func artists(limit: Int = 999, offset: Int = 0, order: ArtistOrder? = nil,
		...
		let response = Network.get(url: url, parameters: parameters, authorization: session.config.authorization, xTidalToken: session.config.apiToken)
...
//			print("tempLimit: \(tempLimit), tempOffset: \(tempOffset)")
	...
	@discardableResult public func removeArtist(artistId: Int) -> Bool {
		...
		let response = Network.delete(url: url, parameters: session.sessionParameters, authorization: session.config.authorization, xTidalToken: session.config.apiToken)
		refreshCachedArtists()
		return response.ok
	}
